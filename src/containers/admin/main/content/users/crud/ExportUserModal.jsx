...
const ExportUserModal = ({ isOpen, onClose, list }) => {
  const handleExport = () => {
    ...
    const exportData = list.map((user, index) => ({
      STT: index + 1,
      ...
      Quyền: user.role === "admin" ? "Admin" : "User",
    }));
  ...
  return (
    <MyModal show={isOpen} handleClose={onClose} title="Xác nhận " size="md">
      <span>Bạn có chắc chắn muốn xuất danh sách người dùng không?</span>
